/**
 * Generated by orval v7.12.2 🍺
 * Do not edit manually.
 * hello world
 * OpenAPI spec version: 1.0.0
 */
import type { ListCategoryResponseFiltersItemLabel } from "./listCategoryResponseFiltersItemLabel";
import type { ListCategoryResponseFiltersItemModel } from "./listCategoryResponseFiltersItemModel";
import type { ListCategoryResponseFiltersItemField } from "./listCategoryResponseFiltersItemField";
import type { ListCategoryResponseFiltersItemAdvanced } from "./listCategoryResponseFiltersItemAdvanced";
import type { ListCategoryResponseFiltersItemEnumOptions } from "./listCategoryResponseFiltersItemEnumOptions";

export type ListCategoryResponseFiltersItem = {
  /** The key representing the service associated with the filter. */
  service: string;
  /** A user-friendly label for the filter. */
  label?: ListCategoryResponseFiltersItemLabel;
  /** Indicates whether the filter is currently open (active). */
  open: boolean;
  /** The type of filter, which determines how it behaves. */
  type: string;
  /** The name of the model to search in. */
  model?: ListCategoryResponseFiltersItemModel;
  /** The scope of the search for this filter, providing context for filtering. */
  field?: ListCategoryResponseFiltersItemField;
  /** Indicates if this filter is an advanced filter. */
  advanced?: ListCategoryResponseFiltersItemAdvanced;
  /** Indicates the filter options that are based on an enumeration. */
  enumOptions?: ListCategoryResponseFiltersItemEnumOptions;
};
