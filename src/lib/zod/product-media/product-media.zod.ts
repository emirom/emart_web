...
export const getProductMediasQueryOrderDefaultOne = "desc";export const getProductMediasQuerySkipMin = 0;
...
export const getProductMediasQueryParams = zod.object({
  "field": zod.union([zod.coerce.string().max(getProductMediasQueryFieldMaxOne),zod.null()]).optional().describe('Field to sort by'),
  ...
  "variantId": zod.union([zod.uuid(),zod.null()]).optional().describe('Filter by specific variant UUID'),
...
export const patchProductMediasIdBody = zod.object({
  "file": zod.any().describe('Media file (image or video) with comprehensive validation'),
